...
func ConvertOpenAIResponseToOpenAI(_ context.Context, modelName string, originalRequestRawJSON, requestRawJSON, rawJSON []byte, param *any) [][]byte {
	if param != nil {
	...
	if !ShouldSanitizeOpenAIVisibleContent(modelName) {
		return [][]byte{rawJSON}
	}
...
// ShouldSanitizeOpenAIVisibleContent scopes the legacy MiniMax duplicated-think
// cleanup to the OpenAI-compatible MiniMax aliases that emit this shape.
// Structured reasoning_content is never modified by this sanitizer.
func ShouldSanitizeOpenAIVisibleContent(modelName string) bool {
	normalized := strings.ToLower(strings.TrimSpace(modelName))
	normalized = strings.TrimPrefix(normalized, "custom:")
	normalized = strings.TrimPrefix(normalized, "cliproxy/")
	switch normalized {
	case "m3", "m3(high)", "m3-high", "minimax-m3", "minimax-m3-high":
		return true
	default:
		return false
	}
